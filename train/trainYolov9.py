...
def get_run_id(output_path='outputs'):
    ...
    id = 1+len(list(output_path.glob('*')))
    return id
...
def gen_patch(args):
    ...
    model = torch.load(MODEL)['model'].float().fuse().eval().to(device)
    ...
    def objectness(outs):
        return torch.cat(outs[0][0])[:,4:].flatten(1).max(1)[0].mean()
        
    def validity(patch):
        return torch.nn.functional.mse_loss(patch, patch.clamp(0.1,0.9))
    
    cudnn.benchmark = True
    def train(epoch=0, save_patches=False):
        outs = []
        def hookfn(modul, inf, outf):
            outs.append(outf)
    
        hook = model.model[-1].register_forward_hook(hookfn)
